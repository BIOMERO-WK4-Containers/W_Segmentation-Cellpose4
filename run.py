...
def process_image(img_path, out_dir, tmp_dir, bj, params):
    """
    Process a 5D image with Cellpose, handling individual dimensions appropriately
    
    Args:
        img_path: Path to the input image
        out_dir: Directory to save final output images (root output directory)
        tmp_dir: Directory for temporary processing files
    ...
    """
    # Create unique subdirectory for this image in tmp directory to avoid conflicts
    img_name = os.path.basename(img_path)
    img_tmp_dir = os.path.join(tmp_dir, f"tmp_{os.path.splitext(img_name)[0]}_{int(time.time())}")
    os.makedirs(img_tmp_dir, exist_ok=True)
    
    # Create separate directory for Cellpose output
    cellpose_output_dir = os.path.join(img_tmp_dir, "cellpose_output")
    os.makedirs(cellpose_output_dir, exist_ok=True)
    ...
    with TiffFile(img_path) as tif:
        ...
        # Get additional parameters with defensive handling
        # Check if 3D processing is needed
        do_3D = getattr(params, 'do_3D', False)
        
        # Get anisotropy parameter
        anisotropy = getattr(params, 'anisotropy', 1.0)
        
        # Check if images should be augmented
        augment = getattr(params, 'augment', False)
        
        # Get norm_percentile values if available
        norm_percentile_min = getattr(params, 'norm_percentile_min', None)
        norm_percentile_max = getattr(params, 'norm_percentile_max', None)
        
        # Edge exclusion option
        exclude_on_edges = getattr(params, 'exclude_on_edges', False)
        
        # GPU device selection
        gpu_device = getattr(params, 'gpu_device', '0')
        
        # Check if normalization should be skipped
        no_norm = getattr(params, 'no_norm', False)          # Building Cellpose command line with updated CLI parameters
        cmd = [
            "conda", "run", "-n", "cellpose_env", "cellpose",  # Use conda run to activate cellpose environment
            "--dir", img_tmp_dir,
            "--savedir", cellpose_output_dir,  # Save results to separate directory
        ...
        # Add GPU options if requested
        if use_gpu:
            cmd.append("--use_gpu")
            cmd.extend(["--gpu_device", gpu_device])
        ...
        if norm_percentile_min is not None and norm_percentile_max is not None:
            cmd.extend(["--norm_percentile", f"{norm_percentile_min}", f"{norm_percentile_max}"])
            
        # Add edge exclusion if requested
        if exclude_on_edges:
            cmd.append("--exclude_on_edges")
        ...
        for metadata in slice_metadata:
            ...
            mask_path = os.path.join(cellpose_output_dir, mask_filename)
...
def main(argv):
    ...
    with BiaflowsJob.from_cli(argv) as bj:
        ...
        for img_index, img_path in enumerate(list_imgs):
            ...
            try:
                # Process image with Cellpose - pass tmp_path for temporary files
                output_path, dimension_mapping = process_image(img_path, out_path, tmp_path, bj, bj.parameters)
